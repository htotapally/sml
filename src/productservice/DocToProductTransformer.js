...
class DocToProductTransformer {
  ...
  Transform (solrresponse) {
    try {
      ...
      docs.forEach(doc => {
        var product = DocToProductTransformer.TransformDoc(doc)
  ...
  static TransformDoc(doc) {
    const unflattenedObject =  DocToProductTransformer.unflattenObject(doc)
...
module.exports = DocToProductTransformer; 
